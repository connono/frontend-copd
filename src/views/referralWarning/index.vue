...
<script>
	...
	import {getWarningPatientCountReferral,getWarningPatientListReferral,deleteWarningPatient,createFollowRecord} from '../../api/warningPatientList'
	@Component({
		components: {
			star: Star
		}
	...
	export default class Warning extends BaseComponent {
	  ...
	  doList(){
		...
		getWarningPatientCountReferral({viewerID: this.$store.state.user.token}).then(response=>{
			var patientTotal = response.data;
			getWarningPatientListReferral({pageIndex: 1, pageOffset: Math.min(10, patientTotal), viewerID: this.$store.state.user.token}).then(response=>{
				//console.log(response);
